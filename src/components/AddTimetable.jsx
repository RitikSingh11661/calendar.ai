...
export const AddTimetable = () => {
  ...
  return (
              ...
              <tr key={rowIndex}>
                {row.map((cell, columnIndex) => {
                  if (columnIndex === 0) {
                    return (
                      <td key={columnIndex}>
                        <input required type="text" value={cell} placeholder="Enter Title" onChange={e=>handleInputChange(rowIndex, columnIndex, e.target.value)}/>
                      </td>
                    );
                  } else if (columnIndex === 1) {
                    const start = cell.split('-')[0],end = cell.split('-')[1];
                    return (
                      <td key={columnIndex}>
                        <label>
                          Start:
                          <input type="time" required onChange={e=>handleInputChange(rowIndex,columnIndex,`${e.target.value}-${end}`)}/>
                        </label>
                        <label>
                          End:
                          <input type="time" required onChange={e=>handleInputChange(rowIndex,columnIndex,`${start}-${e.target.value}`)}/>
                        </label>
                      </td>
                    );
                    ...
                    return (
                      <td key={columnIndex}>
                        <DatePicker
                          ...
                          onChange={date=>handleInputChange(rowIndex, columnIndex, date)}
                ...
                <td>
                  <button className="remove-button" onClick={() => removeRow(rowIndex)}>Remove</button>
                </td>
              </tr>
            );
          })}
    ...
    <div className="buttons-container">
      ...
      <button className="submit-button" onClick={handleSubmit}>Add Schedule</button>
    </div>
  </div>
  );
};